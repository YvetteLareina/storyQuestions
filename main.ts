import { ItemView, WorkspaceLeaf, requestUrl, Plugin, Setting, MarkdownView, PluginSettingTab, Modal } from "obsidian";
// Typdefinitionen und Konstanten
interface StoryQuestionsSettings {
  apiKey: string;
  openaiApiKey: string;  // Neu für OpenAI API
  claudeApiKey: string;  // Neu für Claude API
...
function t(plugin: StoryQuestionsPlugin, key: string): string {
...
async function loadPrompts(app: any, language: "de" | "en"): Promise<Record<string, string[]>> {
...
async function savePrompts(app: any, language: "de" | "en", prompts: Record<string, string[]>) {
  const filePath = ".obsidian/plugins/story-questions/prompts.json";
  let existingContent = "{}";
  try {
...
async function generateAIQuestions(content: string, settings: StoryQuestionsSettings): Promise<string[]> {
  ...
  try {
    ...
    let response: { status: number; json: any }; // Defines response type
    ...
    const questions = generatedText
      ...
      .filter((line: string) => line && line.length > 5)
      .filter((line: string) => !line.includes("Session-ID"))
      .filter((line: string) => !line.includes("plugin:story-questions"))
      .filter((line: string) => !line.includes("Einzigartige"))
      .map((line: string) => {
        let cleanLine = line.replace(/^\d+[\.\)\-]?\s*/, ''); // Removes numbering
        cleanLine = cleanLine.replace(/^[QF]:\s*/, ''); // Removes prefixes
        if (!cleanLine.endsWith("?")) cleanLine += "?"; // Ensures question mark
        return cleanLine;
      })
      .filter((line: string, index: number, self: string[]) => 
        self.indexOf(line) === index) // Removes duplicates
...
class StoryQuestionsView extends ItemView {
  plugin: StoryQuestionsPlugin;
  prompts: Record<string, string[]> = {};
  questionsDiv?: HTMLElement;
  lastDropTime: number = 0;
  lastDropId: string = "";
  processedDrops: Set<string> = new Set();

  constructor(leaf: WorkspaceLeaf, plugin: StoryQuestionsPlugin) {
    super(leaf);
    this.plugin = plugin;
  ...
  registerDropHandler() {
    ...
    if (markdownView) {
      ...
      if (editorEl) {
        const targetEditorEl = editorEl as HTMLElement;
        
        this.registerDomEvent(targetEditorEl, "dragover", (evt: DragEvent) => {
          evt.preventDefault();
          evt.stopPropagation();
         });
  
        this.registerDomEvent(targetEditorEl, "drop", (evt: DragEvent) => {
          const now = Date.now();
  ...
  renderQuestions(container: HTMLElement, categoryFilter: string) {
  ...
  async showRandomQuestion(container: HTMLElement) {
  ...
  async generateQuestions(container: HTMLElement) {
...
class EditQuestionsModal extends Modal {
  plugin: StoryQuestionsPlugin;
  prompts: Record<string, string[]>;
  private tabContainer: HTMLElement; // Nicht mehr optional
  private contentContainer: HTMLElement; // Nicht mehr optional

  constructor(plugin: StoryQuestionsPlugin) {
    super(plugin.app);
    this.plugin = plugin;
    this.prompts = {};
  ...
  async onOpen() {
    ...
    Object.keys(this.prompts).forEach((category, index) => {
      ...
      tab.addEventListener("click", () => {
        this.tabContainer.querySelectorAll(".nav-tab").forEach((t) => (t as HTMLElement).removeClass("active"));
        this.contentContainer.querySelectorAll(".tab-pane").forEach((p) => (p as HTMLElement).removeClass("active"));
        tab.addClass("active");
  ...
  renderQuestions(category: string, list: HTMLElement) {
    list.empty();
  ...
  async editQuestion(category: string, index: number, span: HTMLElement, li: HTMLElement) {
    ...
    const editIcon = li.querySelector(".edit-icon") as HTMLElement | null;
    const deleteIcon = li.querySelector(".delete-icon") as HTMLElement | null;
  ...
  async deleteQuestion(category: string, index: number, li: HTMLElement) {
  ...
  async addQuestion(category: string, list: HTMLElement) {
  ...
  async deleteCategory(category: string, tab: HTMLElement, tabContent: HTMLElement) {
    ...
    yesButton.addEventListener("click", async () => {
      ...
      const firstTab = this.contentEl.querySelector(".nav-tab") as HTMLElement | null;
      const firstTabContent = this.contentEl.querySelector(".tab-pane") as HTMLElement | null;
      if (firstTab && firstTabContent) {
        this.contentEl.querySelectorAll(".nav-tab").forEach((t) => (t as HTMLElement).removeClass("active"));
        this.contentContainer.querySelectorAll(".tab-pane").forEach((p) => (p as HTMLElement).removeClass("active"));
...
class StoryQuestionsSettingTab extends PluginSettingTab {
  plugin: StoryQuestionsPlugin;

  constructor(app: any, plugin: StoryQuestionsPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
